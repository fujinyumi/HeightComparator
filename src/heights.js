...
c["taikogane"] = new Character("Taikogane Sadamune", 0, 24.9, 32.5, "taikogane", 1.0202);
...
//boolean determines if sorted
//true if sorted, false if not
function drawCanvas(bool) {
	...
	if (bool == false) {
		for(var i = 0; i < charanames.length; i++) {
			if (charanames[i] in c) {
				todraw.push(c[charanames[i]]);
			}
			else {
				//error out: this should not happen
				alert("Error: Character of id " + charanames[i] + " not found in database. Please alert this bug to the webmaster.");
			}
		}	
	}
	...
	//sorted
		...
		for(var i = 0; i < charanames.length; i++) {
			if (charanames[i] in c) {
				...
				if (lastOpt == -1)
					return;
				if (lastOpt == 0)
					value = c[charanames[i]].height;
				else if (lastOpt == 1)
					value = c[charanames[i]].length;
				else if (lastOpt == 2)
					value = c[charanames[i]].full;
				
				tosort.push({name: charanames[i], val: value});			
			}
			else {
				alert("Error: Character of id " + charanames[i] + " not found in database. Please alert this bug to the webmaster.");
			}
		...
		tosort.sort(sortHelper);
		
		for(var i = 0; i < tosort.length; i++) {
			todraw.push(c[tosort[i].name]);
		}
	}
...
//comparators for character objects
...
function sortCanvas() {
	...
	for (var key in c) {
		heights.push(c[key].height);
		length.push(c[key].length);
		fulls.push(c[key].full);
	}
